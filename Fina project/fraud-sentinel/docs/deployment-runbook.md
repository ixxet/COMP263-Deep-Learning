...
4. Apply the Talos overlay with Flux or `kustomize build k8s/overlays/talos | kubectl apply -f -`.
5. Wait for `fraud-migrate` to complete.
6. Wait for `fraud-rag-seed` to complete.
7. Run `fraud-trainer` and wait for model artifacts on the PVC.
8. Restart `fraud-api` after training if it started before artifacts existed.
9. Open `fraud-ui` through the internal Cilium/Tailscale path.
